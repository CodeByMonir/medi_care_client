...
export default function ApplyForVerificationPage() {
    ...
    // Added doctorId configuration key inside state blueprint
    ...
    useEffect(() => {
        if (session?.user) {
            ...
            setProfile((prev) => ({
                ...prev,
                doctorId: prev.doctorId || generatedId, // Retain if already present, otherwise apply new
    ...
    const handleSubmit = async (e) => {
        ...
        const doctorData = {
            doctorId: profile.doctorId, // Dispatched cleanly to database collections storage records
            name: session.user.name,
            email: session.user.email,
            phone: profile.phone,
        ...
        try {
            ...
            if (res?.insertedId) {
                ...
                setSuccessMessage(`Your application files under ID ${profile.doctorId} have been recorded and sent for processing.`);
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-white border border-slate-200 rounded-2xl dark:bg-slate-900 dark:border-slate-800 shadow-sm p-6 text-center">
                        ...
                        <div className="flex flex-col items-center justify-center gap-2">
                            ...
                            {/* Clean visual representation layout for Doctor ID badge status summary code item */}
                            <span className="inline-flex items-center gap-1 text-xs font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2.5 py-1 rounded-lg border border-slate-200 dark:border-slate-700">
                                <FaFingerprint className="text-slate-400" /> {profile.doctorId || "Generating..."}
                            </span>
                        </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white border border-slate-200 rounded-2xl dark:bg-slate-900 dark:border-slate-800 shadow-sm p-6 md:p-8 space-y-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-5">
                            {/* Doctor ID Field (Read-only view for assurance layout parameters) */}
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl dark:bg-slate-900 dark:border-slate-800 shadow-sm p-6 md:p-8 space-y-6">
                        ...
                        <div className="grid sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input type="text" name="specialization" value={profile.specialization} onChange={handleInputChange} className={inputClasses} required placeholder="e.g. Pediatrics" />
                            </div>
                            <div>
